import { SQLTable, PrismaModel, PrismaField, SQLParseResult, PrismaEnum, SQLEnum } from "@/types";
...
`;
    ...
    const enumsString = enums.length > 0 ? enums.map(enumDef => this.formatEnum(enumDef)).join('\n\n') + '\n\n' : '';
    const modelsString = models.map(model => this.formatModel(model)).join('\n\n');
    
    return header + enumsString + modelsString;
  }

  private static convertTablesToModels(tables: SQLTable[], enums: PrismaEnum[] = []): PrismaModel[] {
    ...
    for (const table of tables) {
      const model = models.find(m => m.name === this.toPascalCase(table.name))!;
      
      for (const constraint of table.constraints) {
        if (constraint.type === 'FOREIGN KEY' && constraint.referencedTable) {
          // Generate a single relation name for this FK constraint
          const relationName = this.generateUniqueRelationName(table.name, constraint.referencedTable, constraint.columns[0], relationCounter);
          
          // Add forward relation to current model
          const relationField = this.createRelationField(constraint, table, relationName, fieldNameCounters);
          if (relationField) {
            model.fields.push(relationField);
          }

          // Add back-relation to referenced model using the same relation name
          const backRelationField = this.createBackRelationField(constraint, table, relationName, fieldNameCounters);
          if (backRelationField) {
            const referencedModel = models.find(m => m.name === this.toPascalCase(constraint.referencedTable!))!;
            if (referencedModel) {
  ...
  private static createBasicModel(table: SQLTable, enums: PrismaEnum[] = []): PrismaModel {
  ...
  private static convertColumnToField(column: SQLTable['columns'][0], _table: SQLTable, enums: PrismaEnum[] = []): PrismaField {
    const name = this.toCamelCase(column.name);
    const type = this.mapSQLTypeToPrismaType(column.type, column.isEnum ? enums : []);
    const attributes: string[] = [];
    
    if (column.isPrimaryKey) {
      if (column.type === 'SERIAL' || column.type === 'BIGSERIAL') {
        attributes.push('@id @default(autoincrement())');
      } else if (column.type === 'UUID' && column.defaultValue?.includes('gen_random_uuid')) {
    ...
    if (column.defaultValue && !column.isPrimaryKey) {
      ...
      if (upperDefault === 'CURRENT_TIMESTAMP' || upperDefault === 'NOW()' || upperDefault === '(NOW())') {
        ...
        attributes.push(`@default(${column.defaultValue})`);
      } else if (upperDefault.includes('NOW()') || upperDefault.includes('CURRENT_TIMESTAMP')) {
        attributes.push('@default(now())');
      } else if (column.defaultValue.includes('::json') || column.defaultValue.includes('::jsonb')) {
    ...
    if (column.type.includes('TIMESTAMP') && column.name.toLowerCase().includes('updated')) {
  ...
  private static createRelationField(constraint: SQLTable['constraints'][0], table: SQLTable, relationName: string, fieldNameCounters: Map<string, Map<string, number>>): PrismaField | null {
    if (!constraint.referencedTable || !constraint.columns.length || !constraint.referencedColumns) return null;
    
    const referencedModelName = this.toPascalCase(constraint.referencedTable);
    const fieldName = this.generateUniqueFieldName(table.name, constraint.referencedTable, constraint.columns, fieldNameCounters, false);
    
    // Support composite foreign keys
    const foreignKeyFields = constraint.columns.map(col => this.toCamelCase(col));
    const referencedFields = constraint.referencedColumns.map(col => this.toCamelCase(col));
    
    return {
      name: fieldName,
      type: referencedModelName,
      attributes: [`@relation("${relationName}", fields: [${foreignKeyFields.join(', ')}], references: [${referencedFields.join(', ')}])`],
      isOptional: this.isRelationOptional(constraint, table),
      isArray: false
    };
  }

  private static createBackRelationField(constraint: SQLTable['constraints'][0], table: SQLTable, relationName: string, fieldNameCounters: Map<string, Map<string, number>>): PrismaField | null {
    if (!constraint.referencedTable || !constraint.columns.length) return null;
    
    const sourceModelName = this.toPascalCase(table.name);
    const fieldName = this.generateUniqueFieldName(table.name, constraint.referencedTable, constraint.columns, fieldNameCounters, true);
  ...
  private static generateUniqueRelationName(fromTable: string, toTable: string, fkColumn: string, relationCounter: Map<string, number>): string {
    // Simple base relation name
    const baseName = `${this.toPascalCase(fromTable)}To${this.toPascalCase(toTable)}`;
  ...
  private static generateUniqueFieldName(sourceTable: string, targetTable: string, columns: string[], fieldNameCounters: Map<string, Map<string, number>>, isBackRelation: boolean): string {
    ...
    if (isBackRelation) {
      // For back relations (one-to-many), use pluralized source table name
  ...
  private static extractRelationFieldName(fkColumn: string, targetTable: string): string {
  ...
  private static extractForeignKeyContext(fkColumn: string): string {
    ...
    return fkColumn.replace(/_id$/i, '').replace(/^.*_/, ''); // Remove _id and table prefixes
  }

  private static isRelationOptional(constraint: SQLTable['constraints'][0], table: SQLTable): boolean {
  ...
  private static mapSQLTypeToPrismaType(sqlType: string, enums: PrismaEnum[] = []): string {
    // First check if this is an enum type by comparing with enum names
    const expectedEnumName = this.toPascalCase(sqlType);
    const matchingEnum = enums.find(enumDef => enumDef.name === expectedEnumName);
    if (matchingEnum) {
  ...
  private static formatModel(model: PrismaModel): string {
    ...
    const maxFieldLength = Math.max(...model.fields.map(field => field.name.length));
    const maxTypeLength = Math.max(...model.fields.map(field => 
      field.type + (field.isOptional ? '?' : '') + (field.isArray ? '[]' : '')
    ).map(t => t.length));
    
    // Separate scalar fields from relation fields
    const scalarFields = model.fields.filter(field => !this.isRelationField(field));
    const relationFields = model.fields.filter(field => this.isRelationField(field));
    
    // Add scalar fields first
    for (const field of scalarFields) {
      const fieldName = field.name.padEnd(maxFieldLength);
      const fieldType = (field.type + (field.isOptional ? '?' : '') + (field.isArray ? '[]' : '')).padEnd(maxTypeLength);
